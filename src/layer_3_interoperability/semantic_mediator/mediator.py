...
class SemanticMediator:
    ...
    def translate_with_timing(
        ...
        """Tradução semântica com métricas de tempo e cache simulado.

        Realiza a tradução real de ``entity`` para ``target_vocab``
        (Schema.org ↔ GoodRelations) e simula o custo de tempo com base
        na taxa de acerto do cache: *cache hit* ≈ 1–2 ms, *cache miss* ≈
        5–15 ms. Utilizado pelo :class:`SimulationEngine` para gerar
        latências realistas.

        Args:
            entity: Entidade a traduzir.
            target_vocab: Vocabulário de destino.

        Returns:
            Dicionário com:

            * ``translated_data`` — entidade traduzida;
            * ``translation_time`` — tempo de tradução simulado (ms);
            * ``cache_hit`` — se a tradução foi servida pelo cache;
            * ``error`` — se ocorreu erro de tradução (≈ 0,1 %).
        """
        self.translation_stats["total"] += 1

        # Simular hit/miss de cache — tempos Normal(μ, σ) calibrados (ms)
        is_cache_hit = np.random.random() < self.cache_hit_rate
        if is_cache_hit:
            self.translation_stats["cache_hits"] += 1
            translation_time = max(0.1, float(np.random.normal(
                self.hit_time_mean, self.hit_time_std
            )))
        else:
            self.translation_stats["cache_misses"] += 1
            translation_time = max(0.1, float(np.random.normal(
                self.miss_time_mean, self.miss_time_std
            )))

        # Simular erro de tradução (raro: ~0,1 %)
        error = bool(np.random.random() < self.translation_error_rate)

        # Tradução real (estrutural) via OntologyMapper
        translated = self.translate(entity, target_vocab)
